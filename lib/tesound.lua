...
---@param sound string|table A filepath to the sound file (example: "sounds/jump.ogg"), or a list of filepaths. If a list is used, a random sound from that list will be played. Passing a string is shorthand for passing a table with only one item.
---@param tags string[] One or more tags that can be used to identify a sound (Example, single: "music". Example, multiple: {"sfx", "gun", "player"}). Multiple sounds may share tags, or tags may be unique. If omitted, no tags are assigned. Passing a string is shorthand for passing a table with only one item.
---@param volume number A number between 0 and 1 which specifies how loud a sound is. If the sound has a tag which a volume has been specified for, it will multiply this number (ie., if you use the tag "sfx" and volume 0.5, and "sfx" has been tagVolume-ed to 0.6, then the sound will be played at 0.3 volume).
---@param pitch number A number which specifies the speed/pitch the sound will be played it. If the sound has a tag which a pitch has been specified for, it will multiply this number.
---@param func function A function which will be called when the sound is finished playing (it's passed one parameter - a list with the sound's volume and pitch). If omitted, no function will be used.
...
---@param sound string|table See TEsound.play
---@param tags string[] See TEsound.play
---@param n number The number of times the sound will be looped. If omitted, the sound will loop until TEsound.stop is used.
---@param volume number See TEsound.play
...
---@param channel number Determines which channel (number) or tag (string) will be affected. Since sound channels aren't static, it's advisable to use tags. The special sound tag "all" will always affect all sounds.
---@param volume number See TEsound.play. If omitted, effectively resets volume to 1.
...
---@param channel number See TEsound.volume
---@param pitch number See TEsound.play. If omitted, effectively resets pitch to 1.
...
---@param channel number See TEsound.volume
---@param finish boolean If true, the sound will be allowed to finish playing instead of stopping immediately.
...
---@param tag string The channels of all sounds with this tag will be returned.
---@return number[] A list of sound channel numbers.
...
---@param tag string Chooses the tag to check the volume of.
---@return number volume, a number between 0 and 1 (if nil, defaults to 1).
...
---@param tag string See TEsound.findVolume
---@return number pitch, a number between 0 and 1 (if nil, defaults to 1).
